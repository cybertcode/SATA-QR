<div class="flex items-center justify-center gap-1">
    ...
    @if ($isProtected)
        ...
        <span class="text-default-300" title="Rol del sistema, no se puede eliminar">
    ...
    @elseif ($role->users_count > 0)
        {{-- Tiene usuarios activos, no se puede eliminar --}}
        <button type="button" class="btn size-7.5 bg-default-100 text-default-300 cursor-not-allowed"
            title="Tiene {{ $role->users_count }} usuario(s) asignado(s). Retírelos primero."
            x-on:click="
                Swal.fire({
                    title: 'No se puede eliminar',
                    html: 'El rol <strong>{{ e($role->name) }}</strong> tiene <strong>{{ $role->users_count }} usuario(s)</strong> asignado(s).<br><span class=\'text-sm text-gray-500\'>Primero reasigne o retire los usuarios de este rol.</span>',
                    icon: 'info',
    ...
    @else
        {{-- Sin usuarios, se puede eliminar --}}
