        ...
        <div class="card">
            <div class="card-header flex flex-wrap items-center justify-between gap-4">
                <form method="GET" action="{{ route('students.index') }}" class="flex flex-wrap items-center gap-3">
                    <div class="relative">
                        <input class="ps-11 form-input form-input-sm w-full" placeholder="Buscar alumno por DNI o nombre..."
                    ...
                    @if (auth()->user()->isSuperAdmin())
                        <select class="form-input form-input-sm w-full" name="ie" onchange="this.form.submit()">
                    ...
                    @endif
                    <button type="submit" class="btn btn-sm bg-default-200 text-default-600">
                        <i class="size-4" data-lucide="filter"></i>
                    </button>
                </form>
                <div class="flex items-center gap-2">
                    <a href="{{ route('students.import') }}"
                        class="btn btn-sm bg-info/10 text-info hover:bg-info hover:text-white">
                        ...
                        <div class="overflow-hidden">
                            <table class="min-w-full divide-y divide-default-200">
                                ...
                                <tbody class="divide-y divide-default-200">
                                    @forelse($students as $student)
                                        <tr class="text-default-800 font-normal hover:bg-default-50 transition-colors">
                                            ...
                                            <td class="px-3.5 py-2.5 whitespace-nowrap text-sm">
                                                ...
                                                @if ($matricula)
                                                    <div
                                                        class="inline-flex py-0.5 px-2.5 rounded text-xs font-semibold bg-default-100 border border-default-200 text-default-600">
                                            ...
                                            <td class="px-3.5 py-2.5 whitespace-nowrap">
                                                @if ($matricula)
                                                    <span
                                                        class="inline-flex items-center gap-1.5 text-xs font-medium {{ $matricula->estado === 'Activo' ? 'text-success' : 'text-default-400' }}">
                                                        <span
                                                            class="size-1.5 rounded-full {{ $matricula->estado === 'Activo' ? 'bg-success' : 'bg-default-300' }}"></span>
                                                        {{ $matricula->estado }}
                                                    </span>
                                                @else
                                                    <span
                                                        class="inline-flex items-center gap-1.5 text-xs font-medium text-default-400">
                                                        <span class="size-1.5 rounded-full bg-default-300"></span> Sin
                                                        matrícula
                                                    </span>
                                                @endif
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center py-10 text-default-500">
